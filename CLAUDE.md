...
9. **Минимизация ПДн (Закон РБ № 99-З).** Розница: обязателен только e-mail; имя для сертификата не хранится (Certificate.holderName заполняется вручную при выдаче), телефон/прочее — опционально. **B2B: персональных данных работников платформа не получает вовсе** — учётку создаёт ответственный представитель клиента, без e-mail/ФИО/телефона, идентификатор — сгенерированный логин `<slug>-0042` (имя работника, если клиенту оно нужно, шифруется в браузере ПИН-кодом организации — сервер хранит непрозрачный blob); оператором остаётся клиент (оферта `/offer-b2b` п. 10, Приложение 1). Поля `name/email/phone` в B2B-действиях запрещены на уровне zod-схем, а не только скрыты в UI. Бэкапы шифруются. ПДн не логировать.

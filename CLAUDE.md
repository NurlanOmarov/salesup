...
Платформа онлайн-курсов по продажам, работающая **без участия преподавателя**: «фабрика курсов» (CLI) собирает курсы из YouTube-записей, AI-критик валидирует контент, AI-наставник отвечает ученикам, владелец читает еженедельный дайджест. В MVP **нет онлайн-оплаты**: админ вручную создаёт ученика и выдаёт логин/пароль после устного подтверждения оплаты. Видео и все сервисы — на одном VPS. Рынки: Беларусь (решение владельца, 2026-07-16; применимое право — Закон РБ от 07.05.2021 № 99-З, регулятор НЦЗПД), Казахстан, Россия и Узбекистан — **четыре домена на одном сервисе** (`study.activesales.by` / `study.activesales.kz` / `study.sales-active.ru` / `study.activesales.uz`, D-013 и docs/MULTI-DOMAIN-PLAN.md): самостоятельные сессии, self-canonical + hreflang на каждом домене, страновые приложения к оферте и политике. Язык интерфейса: русский; субтитры видео: RU / KK / EN / UZ. Помимо розницы есть B2B-контур: организация покупает места в лицензиях, её работники учатся обезличенно (docs/B2B-PLAN.md, оферта `/offer-b2b`).
...
- **E-mail (некритично в MVP)**: nodemailer → SMTP. Доступы выдаются админом лично; письма: доступы покупателю и клиенту-организации, ссылка сброса пароля (самообслуживание для тех, у кого есть e-mail: розница и ответственный организации; работникам организаций без e-mail пароль сбрасывает ответственный) — выключаются флагом `EMAIL_ENABLED`
...
9. **Минимизация ПДн (Закон РБ № 99-З).** Розница: обязателен только e-mail, телефон/прочее — опционально. **ФИО — только для сертификата и только от самого ученика**: вводит на шаге 2 страницы сертификатов с отдельной галочкой согласия (`lib/certificates/consent.ts`, `Certificate.consentAt/consentVersion`; D-019). **B2B: клиент ФИО работников не вносит** — учётку создаёт ответственный представитель, без e-mail/ФИО/телефона, идентификатор — логин `<slug>-0042` (подпись — кличка/должность открытым текстом, без ФИО по п. 10.1 оферты; D-017); оператором ПДн работников остаётся клиент (оферта `/offer-b2b` п. 10, Приложение 1), кроме ФИО для сертификата, которое работник даёт сам (п. 10.5). Поля `name/email/phone` в B2B-действиях организации запрещены на уровне zod-схем, а не только скрыты в UI. Бэкапы шифруются. ПДн не логировать.

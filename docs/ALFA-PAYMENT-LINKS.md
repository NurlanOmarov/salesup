...
Проверка на одном курсе пройдена, дальше — одинаковая работа по каждому из девяти.
На курс уходит около трёх минут.
...
Сейчас идёт скидка −50 % до **30 сентября 2026** включительно, поэтому ссылки создаются
...
> ⚠️ **1 октября 2026** акция заканчивается, витрина сама возвращается к полным ценам —
> и суммы во всех девяти ссылках нужно поднять до полных. Это единственная ручная работа,
> которую акция за собой тянет; поставь напоминание.

### 15.2. Таблица: что создавать

Для каждого курса — **Название**, **Сумма**, **Описание**. Остальные поля одинаковы:
ограничение «Без ограничений», язык «Русский», тип оплаты «Одностадийная», в дополнительных
параметрах включена только **Электронная почта**.

| Курс (название ссылки) | Сумма сейчас (акция) | Сумма с 1 октября | Описание ссылки |
|---|---|---|---|
| Эффективные продажи кухонь 2.0 | **295** | 590 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-kitchens` |
| Техники продаж в туризме | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-tourism` |
| B2B-переговоры и крупные сделки | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-b2b` |
| Техники продаж недвижимости | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-realty` |
| Активные продажи для медпредставителей | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-pharma` |
| Продажи в магазине обуви и одежды | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-shoes` |
| Продажи в DIY-магазине | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-diy` |
| Тайм менеджмент: базовые принципы | **160** | 320 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: time-management` |
| СПИН-продажи: как формировать потребность | **125** | 250 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-spin` |

Все описания укладываются в лимит 100 символов (самое длинное — 72). Код курса обязан быть
целиком: по нему платформа опознаёт, что оплачено.

**Ссылка на СПИН-продажи уже существует** — её не создают заново, а редактируют: сумму 1 Br
меняют на 125 Br и дописывают код курса в описание, если его там ещё нет.
...
**Шаг 7.** Повторить шаги 1–6 для остальных курсов.
...
переписывать не нужно — это и позволит 1 октября поднять суммы, не трогая платформу.

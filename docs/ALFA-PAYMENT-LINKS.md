...
Курс опознаётся по **коду в описании ссылки** — `sales-kitchens`, `sales-spin` и так далее.
Это адрес курса на платформе, он не меняется никогда. Описание едет вместе с платежом и
приходит в уведомлении об оплате, поэтому **курс определяется тем, на какую ссылку человек
нажал, а не тем, что он делал на платёжной странице**.

> **Почему именно описание, а не отдельный параметр.** При создании ссылки можно завести свой
> параметр `course` — и он действительно виден на предплатежной странице. Но в callback-уведомление
> пользовательские параметры не попадают: их список у шлюза фиксирован (`payerEmail`,
> `orderDescription`, `amount`, `orderNumber` и прочие системные поля). Название ссылки в
> уведомление тоже не приходит, хотя в настройках его выбрать можно. Проверено на боевой оплате
> 07.09.2026: единственное поле, куда можно положить своё и получить обратно, — описание.
> Отсюда же требование к его длине: **описание обрезается на 100 символах**, и код курса должен
> уместиться целиком (раздел 6.1).
...
2. Предплатежная страница банка: покупатель вводит ivanov@mail.by
   Банк создаёт заказ №30412 с описанием ссылки:
        сумма    = 590.00 BYN
        описание = «Доступ на 12 месяцев… Код: sales-kitchens»
...
4. Банк присылает уведомление на study.activesales.by/api/payments/alfa
        mdOrder          = 1c1ccf26-7728-…   (номер заказа в банке)
        orderNumber      = 30412
        operation        = deposited         (деньги списаны)
        status           = 1                 (успешно)
        amount           = 59000             (в копейках)
        payerEmail       = ivanov@mail.by
        orderDescription = «Доступ на 12 месяцев… Код: sales-kitchens»
        checksum         = 9B7666BC8ADE…     (подпись)

5. Платформа проверяет подпись — убеждается, что уведомление действительно от банка

6. Ищет в описании код курса → sales-kitchens → «Эффективные продажи кухонь 2.0»
   Находит ученика по ivanov@mail.by → нет такого → создаёт
   Открывает доступ на 12 месяцев, отправляет письмо с временным паролем
```

Всё нужное приходит в самом уведомлении, дополнительный запрос к API не требуется. Запасной путь
через `getOrderStatusExtended.do` в платформе есть и включается сам, если доступы к API заданы, —
но в обычной работе он не нужен.
...
Форма открывается по кнопке **Создать** и выглядит так — сверху вниз:

| Поле формы | Что ставим | Почему |
|---|---|---|
| **Название** | название курса: «Эффективные продажи кухонь 2.0» | под полем написано «Отображается на платёжной странице» — это **видит покупатель** |
| **Описание** | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-kitchens` | тоже видит покупатель; код курса здесь обязателен — см. предупреждение ниже |
| **Язык** | Русский | язык платёжной страницы |
| **Ограничение** | точка у **«Без ограничений»** | ссылка постоянная: одна на курс, работает всегда. «Период» и «Количество платежей» нужны для разовых счетов |
| **Сумма** | точка у **«Фиксированная»**, в поле — число из таблицы 15.2 | «Любая» позволила бы покупателю ввести свою сумму — для курса это лишнее |

Поля «Валюта» и «Тип оплаты» в форме нет: валюта берётся из настроек мерчанта (BYN), а выбор
одно/двухстадийной оплаты появляется, только если двухстадийная включена банком. У нас она
выключена, и это правильно: услуга открывается сразу, удерживать деньги незачем.

Сумму вводим числом без пробелов и валюты: `295`, а не `295 BYN`.

> ⚠️ **Описание — ровно 100 символов, лишнее обрезается молча.** Проверено на боевой ссылке:
> строка «Онлайн-курс, доступ на 12 месяцев. Доступ откроется автоматически на указанный e-mail.
> Код курса: sa» обрубилась на сотом символе, и код курса потерялся. Поэтому текст описания
> держим коротким, а **код курса пишем в конце и целиком**: именно по нему платформа понимает,
> какой курс оплачен (раздел 2.7). Самый длинный код — `time-management` (15 символов), так что
> под остальной текст остаётся около 80.
>
> Название и описание видит покупатель на платёжной странице, поэтому «Код: sales-spin» читается
> как обычный артикул товара — это нормально.
...
Свой параметр через **«Добавить параметр»** (`course` = адрес курса) завести можно — покупатель
увидит его на предплатежной странице, и он останется в карточке заказа. Но **в callback-уведомление
он не попадает**: список дополнительных параметров уведомления фиксирован и пользовательских полей
не содержит. Проверено на боевой оплате — поэтому опознание курса держится на коде в описании
(раздел 2.7), а не на этом параметре.
...
Название ссылки — название курса. Описание — строка из таблицы целиком: она укладывается в
лимит 100 символов вместе с кодом курса.

| Курс | Сумма | Описание ссылки (символов) |
|---|---|---|
| Эффективные продажи кухонь 2.0 | 590 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-kitchens` (71) |
| Техники продаж в туризме | 490 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-tourism` (70) |
| B2B-переговоры и крупные сделки | 490 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-b2b` (66) |
| Продажи в магазине обуви и одежды | 350 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-shoes` (68) |
| Техники продаж недвижимости | 350 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-realty` (69) |
| Активные продажи для медпредставителей | 350 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-pharma` (69) |
| Тайм менеджмент: базовые принципы | 320 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: time-management` (72) |
| СПИН-продажи: как формировать потребность | 250 BYN | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-spin` (67) |
...
## 15. Запуск всех курсов белорусской витрины

Проверка на одном курсе пройдена, дальше — одинаковая работа по каждому курсу витрины.
Сейчас их **12**; на курс уходит около трёх минут.

### 15.1. Главное: сумма в ссылке должна равняться цене на витрине

Сумма зашита в платёжную ссылку на стороне банка, а цена на витрине живёт на платформе —
и **во время акции витрина показывает половину прайса** (`src/lib/pricing/promo.ts`).
Сейчас идёт скидка −50 % до **31 октября 2026** включительно, поэтому ссылки создаются
на акционные суммы. Создашь по полному прайсу — покупатель увидит 295 Br, а спишется 590.

Платформа за этим следит: если оплаченная сумма не совпадает с ценой витрины, владельцу
приходит уведомление «Цена в банке и на витрине разошлись» с номером заказа. Доступ при этом
всё равно открывается — человек не должен ждать, пока разберутся с суммой.

> ⚠️ **1 ноября 2026** акция заканчивается, витрина сама возвращается к полным ценам —
> и суммы во всех ссылках нужно поднять до полных (столбец «С 1 ноября»). Это единственная
> ручная работа, которую акция за собой тянет; поставь напоминание. Если акцию снова продлят,
> дату окончания смотри в `PROMO.endsAt` — таблица ниже от неё зависит.

### 15.2. Таблица: что создавать

Состояние на 22 сентября 2026. Для каждого курса — **Название**, **Сумма**, **Описание**.
Остальные поля одинаковы: ограничение «Без ограничений», язык «Русский», сумма «Фиксированная»,
в дополнительных параметрах включена только **Электронная почта**.

| Название ссылки | Сумма сейчас (акция) | С 1 ноября | Описание ссылки | Ссылка |
|---|---|---|---|---|
| Эффективные продажи кухонь 2.0 | **295** | 590 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-kitchens` | создать |
| Кухни — основа продаж | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-kitchens-basics` | создать 🆕 |
| Техники продаж в туризме | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-tourism` | создать |
| B2B-переговоры и крупные сделки | **245** | 490 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-b2b` | создать |
| Активные продажи для медицинских представителей | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-pharma` | создать |
| Продажи в магазине обуви и одежды | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-shoes` | создать |
| Продажи в DIY-магазине | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-diy` | создать |
| Техники продаж недвижимости | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-realty` | создать |
| Клиентский сервис в ломбарде | **175** | 350 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: service-pawnshop` | создать 🆕 |
| Тайм менеджмент: базовые принципы | **160** | 320 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: time-management` | создать |
| Продажи матрасов в рознице | **145** | 290 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-mattresses` | создать 🆕 |
| СПИН-продажи: как формировать потребность | **125** | 250 | `Доступ на 12 месяцев откроется на указанный e-mail. Код: sales-spin` | есть — отредактировать |

🆕 — курсы, добавленные после первой версии инструкции.

Все описания укладываются в лимит 100 символов (самое длинное — 78). Код курса обязан
быть целиком: по нему платформа опознаёт, что оплачено. Коды, начинающиеся одинаково
(`sales-kitchens` и `sales-kitchens-basics`), платформа различает — это проверено тестом.

**Ссылка на СПИН-продажи уже существует** — её не создают заново, а редактируют: сумма должна
быть 125 Br (при тесте ставили 1 Br), а в конце описания — `Код: sales-spin`.

**Новый курс на витрине → новая ссылка.** Сумма = цена курса с учётом акции, описание — по
образцу из таблицы с кодом курса (это адрес курса: `study.activesales.by/courses/<код>`).
Пока ссылки у курса нет, он продаётся через заявку — ничего не ломается.

### 15.3. Как заполнять форму в кабинете — по шагам

Разбор на курсе «Эффективные продажи кухонь 2.0»; для остальных меняются только название,
сумма и код в описании (таблица 15.2).

**Шаг 1. Открыть форму.** Кабинет `ecom.alfabank.by/generalmp3` → левое меню **Инструменты
оплаты → Ссылки на оплату** → красная кнопка **«Создать»** вверху (или «+ Создать» → «Ссылку
на оплату»).

**Шаг 2. Заполнить поля сверху вниз.**
...
│  Название                                                  │
│  ┌──────────────────────────────────────────────────────┐ │
│  │ Эффективные продажи кухонь 2.0                       │ │  ← видит покупатель
│  └──────────────────────────────────────────────────────┘ │
│  Отображается на платёжной странице                        │
│                                                            │
│  Описание                                                  │
│  ┌──────────────────────────────────────────────────────┐ │
│  │ Доступ на 12 месяцев откроется на указанный e-mail.  │ │  ← видит покупатель
│  │ Код: sales-kitchens                                  │ │  ← по нему платформа
│  └──────────────────────────────────────────────────────┘ │     узнаёт курс
│  Отображается на платёжной странице        (лимит 100 симв.)│
│                                                            │
│  Язык        [ Русский ▾ ]                                 │
│                                                            │
│  Ограничение                                               │
│    (•) Без ограничений        ← ставим сюда                │
│    ( ) Период                                              │
│    ( ) Количество платежей                                 │
│                                                            │
│  Сумма                                                     │
│    (•) Фиксированная   [ 295 ]   ← число без валюты        │
│    ( ) Любая                                               │
│                                                            │
│  Дополнительные параметры                              [∨] │  ← раскрыть стрелкой
│    [ ] Адрес                                               │
│    [✓] Электронная почта      ← единственная галочка       │
│    [ ] ФИО                                                 │
│    [ ] Телефон                                             │
│    + Добавить параметр        ← пропускаем, см. 6.2        │
│                                                            │
│                        [ Отмена ]   [ Создать ]            │
└────────────────────────────────────────────────────────────┘
```

Про поля, которых в форме нет: **валюта** берётся из настроек мерчанта (BYN), а выбор
одно/двухстадийной оплаты появляется, только если банк включил двухстадийную. У нас её нет,
и это правильно — услуга открывается сразу, удерживать деньги незачем.

**Шаг 3. Нажать «Создать».** Ссылка появится в списке со статусом **Активна**.

**Шаг 4. Скопировать адрес.** Нажать на строку ссылки → кнопка **«Копировать»** (рядом есть
«Поделиться» и «Скачать QR-код»). Адрес будет вида `https://ecom.alfabank.by/sc/XXXXXXXXXXXX`.

**Шаг 5. Прописать на платформе.** `study.activesales.by/admin/courses` → нужный курс → поле
**«Ссылка на оплату (Альфа-Банк)»** → вставить адрес → **Сохранить**. Платформа принимает только
адреса на домене `alfabank.by`: чужая ссылка в этом поле означала бы увод оплаты на сторону.

**Шаг 6. Проверить.** Открыть страницу курса на сайте: под заголовком должна появиться кнопка
**«Оплатить и начать»**. Нажать её и сверить на странице банка две вещи — название курса и
сумму. Они должны совпадать с тем, что показывает витрина.

**Шаг 7.** Повторить шаги 1–6 для остальных курсов из таблицы 15.2.

### 15.4. Если нужно поправить уже созданную ссылку

Щёлкнуть по ссылке в списке → откроются её детали → **Редактировать**. Меняется всё: название,
описание, сумма. **Адрес ссылки при этом не меняется**, поэтому в админке платформы ничего
переписывать не нужно — это и позволит 1 ноября поднять суммы, не трогая платформу.

Там же, через меню из трёх точек в строке, ссылку можно **деактивировать** или удалить —
например, если курс снят с продажи.

Кнопка показывается только на белорусском домене `study.activesales.by`: договор эквайринга
заключён на белорусскую площадку, цены в ссылках — в белорусских рублях. На `.kz`, `.ru` и `.uz`
курс по-прежнему продаётся через заявку.

### 15.5. Что происходит после запуска

| Событие | Что делает платформа |
|---|---|
| Оплата | открывает доступ на 12 месяцев, шлёт покупателю письмо, владельцу — сообщение о продаже |
| Повторная оплата того же курса | доступ продлевает, владельцу пишет «похоже на случайную оплату, проверьте возврат» |
| Сумма разошлась с витриной | доступ открывает, владельцу пишет о расхождении |
| Возврат в кабинете банка | доступ отзывает |
| Оплата курса без кода в описании | доступ не открывает, зовёт владельца разобраться вручную |

---

## 16. Какой канал оплаты для какого случая

...
**S6.4 🟥 Предзапуск-чеклист.** Lighthouse ≥ 90; e2e зелёные; rate-limit auth/ai; security-заголовки (CSP), TLS A-рейтинг; ~~оферта/политика (с учётом ПДн!)~~ → тексты под право РБ готовы (D-012, `src/content/legal`), остаётся: заполнить реквизиты ИП в `requisites.ts` и выставить `REQUISITES_FILLED = true`, вычитка юристом, перенос хостинга в РБ (Указ № 60); 404/500; фабрика прогнана на 1 реальном курсе; «боевая» выдача доступа реальному ученику; бета 10–20 человек.
...
**S7.5 🟨 Онлайн-оплата — через магазин activesales.by (D-014).** Своего чекаута не будет: карты принимает WooCommerce на основном домене (эквайринг Альфа-Банка уже подключён), платформе приходит подписанный webhook → WebhookEvent → Order PAID → авто-Enrollment(PURCHASE) → письмо с временным паролем. Сделано на стороне платформы: `/api/payments/woo`, `lib/payments/woo/*`, ночная сверка `woo.reconcile`, поле «Товар в магазине» в админке курса, кнопка «Купить и начать» на белорусской витрине. **Осталось (внешний подрядчик по WordPress, инструкция — `docs/WOO-INTEGRATION.md`):** артикулы SKU = slug, прайс по `lib/pricing`, товар для СПИН-курса, вебхук + секрет, ключи REST API, SMTP-ящик. Ручная выдача остаётся параллельным каналом (B2B «по счёту», ЕРИП).

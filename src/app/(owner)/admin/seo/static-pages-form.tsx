          ...
          {page.hasBody ? (
            <div>
              ...
              <textarea
                ...
                placeholder="Пусто → показывается актуальная редакция документа из кода (src/content/legal)"
                value={body}
                onChange={(e) => setBody(e.target.value)}
              />
              <p className="mt-1 text-xs text-foreground/40">
                Пустое поле — рекомендуемый режим: страница показывает выверенную
                редакцию из кода, куда реквизиты подставляются автоматически. Текст
                здесь перекрывает её целиком, включая обновления, — очистите поле,
                чтобы вернуться к актуальной редакции. AI-черновик не является

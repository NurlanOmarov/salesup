          ...
          {page.hasBody ? (
            <div>
              ...
              <textarea
                ...
                placeholder="Пусто → актуальная редакция из кода (src/content/legal) + страновое приложение по домену"
                value={body}
                onChange={(e) => setBody(e.target.value)}
              />
              <p className="mt-1 text-xs text-foreground/40">
                Пустое поле — рекомендуемый режим: страница показывает выверенную
                редакцию из кода, куда реквизиты подставляются автоматически, а на
                доменах .kz и .ru добавляется страновое приложение. Текст здесь
                перекрывает её целиком — включая обновления и страновые приложения,
                причём сразу на всех трёх доменах. Очистите поле, чтобы вернуться к
                актуальной редакции. AI-черновик не является юридической консультацией.
              </p>
            </div>
          ) : null}

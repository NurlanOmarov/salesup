...
const SYSTEM =
  "Ты SEO-редактор онлайн-платформы с курсами по продажам (рынок Беларусь, язык — " +
...
export type StaticDraftPage = "/offer" | "/offer-b2b" | "/privacy";
...
const DRAFT_SPECS: Record<StaticDraftPage, { name: string; brief: string }> = {
  ...
  "/offer-b2b": {
    name: "Публичная оферта для организаций",
    brief:
      "Договор публичной оферты для юридических лиц и ИП: предмет (места в лицензии на " +
      "курс для работников заказчика), персональная учётная запись каждому работнику и " +
      "запрет общих учёток, оплата по счёту и приёмка по акту, отчётность в обезличенном " +
      "виде, интеллектуальная собственность, персональные данные работников (оператор — " +
      "заказчик, платформа — уполномоченное лицо) и поручение на обработку, " +
      "ответственность, расторжение и возврат, споры в экономическом суде.",
  },
  "/privacy": {
    ...
    brief:
      ...
      "правовые основания, сроки хранения, права субъекта и сроки их реализации " +
      "(5 рабочих дней на предоставление информации, 15 дней на прочие заявления), " +
      "уполномоченные лица (хостинг, LLM-провайдеры, почтовый сервис, счётчики) и " +
      "трансграничная передача, cookie и согласие на аналитику, контакт для обращений.",
  },
};

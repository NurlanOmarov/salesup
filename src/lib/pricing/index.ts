...
// ────────────────────── B2B: пакет с живым тренером ──────────────────────

/**
 * Надбавка за работу тренера поверх доступа к платформе (решение владельца,
 * 2026-08-21). Три позиции по 100 $ — фиксированные суммы НА КОМПАНИЮ, а не на
 * сотрудника: вводная сессия ведётся для всей группы разом, итоговая тоже, а
 * чат сопровождения — один на отдел. Умножать их на число мест было бы нечестно
 * и мгновенно выбило бы предложение из рынка.
 *
 * Цены заданы в долларах намеренно: час тренера — это его ставка, одинаковая
 * для Минска, Алматы, Москвы и Ташкента. В валюту витрины сумма переводится по
 * курсу (lib/currency `usdToTiyn`), поэтому на всех четырёх доменах компания
 * платит за тренера одно и то же.
 */
export interface TrainerPackItem {
  key: "intro" | "final" | "group";
  usd: number;
}

export const TRAINER_PACK: readonly TrainerPackItem[] = [
  { key: "intro", usd: 100 },
  { key: "final", usd: 100 },
  { key: "group", usd: 100 },
] as const;

/** Итого за пакет тренера, $. */
export const TRAINER_PACK_USD = TRAINER_PACK.reduce((sum, i) => sum + i.usd, 0);

...
export async function requireOrgAdmin(orgId?: string): Promise<OrgContext> {
  ...
  // Чужой orgId в адресе — уводим в собственный кабинет. Без этой проверки
  // страница открывалась бы под чужим URL (пусть и со своими данными), и любое
  // место, где id берут из параметра, а не из контекста, стало бы утечкой.
  if (orgId && orgId !== membership.org.id) {
    redirect(`/org/${membership.org.id}`);
  }

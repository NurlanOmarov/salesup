 ...
 *
 * Почему не на сервере, как экспорт админ-аналитики: подписи сотрудников
 ...
 *
 * Поэтому: данные приходят с сервера в кодах, метки расшифровываются в браузере,
...
export interface MemberReportRow {
  login: string;
  /** Уже расшифрованная подпись или null, если ключ не введён. */
  label: string | null;
  group: string | null;
  courses: number;
  lessonsDone: number;
...
export interface OrgReportInput {
  orgName: string;
  generatedAt: string;
  /** Были ли подписи расшифрованы — от этого зависит колонка «Сотрудник». */
...
export async function downloadOrgReportXlsx(input: OrgReportInput): Promise<void> {
  ...
  const lines = input.labelsIncluded
    ...
    : [
        "Подписи сотрудников не включены: парольная фраза не вводилась.",

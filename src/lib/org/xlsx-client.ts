/**
 * Сборка XLSX-отчёта организации в браузере: данные уже есть на странице,
...
export interface MemberReportRow {
  login: string;
  /** Подпись работника от ответственного или null. */
  label: string | null;
  group: string | null;
  courses: number;
  lessonsDone: number;
...
export interface OrgReportInput {
  orgName: string;
  generatedAt: string;
  /** Есть ли у кого-то подпись — от этого зависит колонка «Сотрудник». */
...
export async function downloadOrgReportXlsx(input: OrgReportInput): Promise<void> {
  ...
  for (const line of [
    "Сотрудники указаны кодами и подписями, которые вы им дали в кабинете.",
    "",

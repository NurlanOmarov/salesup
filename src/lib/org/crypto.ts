/**
 * L2: шифрование имён работников на стороне клиента (docs/B2B-PLAN.md §5.2).
 *
 ...
 *    зашифрованные под ключом, выведенным из ПИН-кода конкретного
 *    ответственного представителя, и под recovery-кодом;
 *  • метка сотрудника (`OrgMembership.labelEnc`) — непрозрачный blob.
 *
 * Почему не выводим ключ шифрования прямо из кода: тогда смена кода означала
 * бы перешифровку всех имён, а второй ответственный был бы невозможен. Обёртки
...
/** Параметры вывода ключа из ПИН-кода. Хранятся рядом с каждой обёрткой,
...
/** Новый ключ организации. Extractable — иначе его нельзя обернуть под код. */
...
/** Ключ шифрования ключа (KEK) из ПИН-кода или recovery-кода. */
...
/** Обернуть ключ организации под ПИН-код (или recovery-код). */
...
/**
 * Развернуть ключ организации. Неверный код даёт ошибку аутентификации
 * AES-GCM, а не «мусорный ключ» — поэтому проверять код отдельно не нужно.
 */
...
/** Зашифровать метку сотрудника. Пустая строка — это «имена нет» (null). */
...
/**
 * Код восстановления вида `AB2CD-EF3GH-...`. Единственный способ вернуть имена,
 * если ответственный забыл ПИН-код, поэтому показывается один раз и
...
/** Ниже этой длины секрет не принимается. */
export const MIN_PIN_LENGTH = 4;

/**
 * Минимальные требования к ПИН-коду, которым шифруются имена работников.
 *
 * Порог в четыре знака — решение владельца (2026-08-21): код должен держаться
 * в голове у одного человека у клиента, иначе имена просто не заводят. Плата
 * известна и принята: четырёхзначный код — это 10 000 вариантов, и тот, у кого
 * окажется выгрузка OrgKeyWrap, переберёт их офлайн даже через PBKDF2 с 600 000
 * итераций. Онлайн-перебора нет (обёртка не отдаётся никому, кроме своего же
 * ответственного), открытым текстом имена не лежат нигде, а длиннее — надёжнее,
 * о чём форма и говорит. Отсюда же формулировки в UI: «мы их не читаем», а не
 * «прочитать невозможно».
 */
export function validatePin(value: string): string | null {
  const v = value.trim();
  if (v.length < MIN_PIN_LENGTH) {
    return `Код должен быть не короче ${MIN_PIN_LENGTH} знаков`;
  }
  return null;
}
